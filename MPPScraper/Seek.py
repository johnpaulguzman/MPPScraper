...
def search_jobs(keywords, page_buffer=5):
    search_cache = os.path.join(*(Config.data_path, keywords, "search_jobs.json"))
    ...
    search_replace_dict = {
        "<KEYWORDS>": [urllib.parse.quote_plus(keywords)],
        "<PAGE>": list(range(1, pages + 1))[ :Config.max_pages], }
    search_workers = 36
...
def view_posts(keywords, ids):
    posts_cache = os.path.join(*(Config.data_path, keywords, "view_posts.json"))
...
def view_applications(keywords, ids):
    applications_cache = os.path.join(*(Config.data_path, keywords, "view_applications.json"))
...
if __name__ == '__main__':
    ...
    """ TODO: FIX DUPLICATE DUMP ENTRIES
melbid = 1002
dump_cache = os.path.join(*(Config.data_path, keywords, "dump.json"))
    """
